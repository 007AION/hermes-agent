...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        # Pin the curated manifest so the test is deterministic regardless
        # of what the live manifest server returns.  Without this the test
        # depends on whether the test runner can reach the manifest endpoint.
        manifest = [
            ("anthropic/claude-opus-4.8", ""),
            ("qwen/qwen3.7-max", ""),
            ("nvidia/nemotron-3-super-120b-a12b:free", "free"),
        ]
        with patch("hermes_cli.model_catalog.get_curated_openrouter_models", return_value=manifest), \
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        # Pin the curated manifest so this test is deterministic.
        manifest = [
            ("anthropic/claude-opus-4.8", ""),
            ("qwen/qwen3.7-max", ""),
        ]
